...
a.page, 
a.page:link { color: gray; text-decoration: none;}
a.page:visited { color: gray; text-decoration: none;}
a.page:hover { color: gray; font-weight:bold; text-decoration: none;}
a.page:active { color: gray; text-decoration: none; }
						...
						<div class="l" style="margin-top:21px; width:672px; height:max-height; margin-left:20px; padding: 0 50px 50px 50px">
							...
							<table class="comTable" style="width:652px; margin-top:5px; margin-bottom:5px;">
								...
								<c:forEach var="list" items="${boardList }">
									<tr>
										<td>${list.free_board_no}</td>
										<td class="title"><a class="name" href="/community/freeboard_content.do?free_board_no=${list.free_board_no}">${list.free_board_title}</a>
										<c:if test="${list.free_board_reply_cnt != 0 }">
										(${list.free_board_reply_cnt})
										</c:if>
										</td>
										<td>${list.free_board_writer_nick}</td>
										<td>${list.free_board_hit}</td>
									</tr>
								</c:forEach>							
							</table>
							<div style="width:max-width; text-align: right; padding-right:10px;">
							<button type="button" style="width:60px; height:30px;" onclick="location.href='/community/freeboard_write.do'">글쓰기</button>
							</div>
